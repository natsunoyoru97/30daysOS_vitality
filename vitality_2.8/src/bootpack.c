...
void HariMain(void)
{
	...
	*/
	int key_to = 0, key_shift = 0, key_option = 0, key_leds = (binfo->leds >> 4) & 7, keycmd_wait = -1;
	struct CONSOLE *cons;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (i == 256 + 0x0f) { //Tab
				...
				if (i == 256 + 0x10 && key_shift != 0 && task_cons->tss.ss0 != 0) {	// Shift+Q
				...
				if (i != 0) {
					timer_init(timer, &fifo, 0); //下次设定为0
					if (cursor_c >= 0) {
						cursor_c = COL8_000000;
					}
				} else {
					timer_init(timer, &fifo, 1); //下次设定为1
					if (cursor_c >= 0) {
						cursor_c = COL8_FFFFFF;
					}
				}
